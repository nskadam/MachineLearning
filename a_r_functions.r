...
if(TRUE){
  if(FALSE){
    IRLBA <- function (A, nu = 5, nv = 5, adjust = 3, aug = c("ritz", "harm"), 
      ...
      while (iter <= maxit) {
        cat('Working on itteration: ', iter, 
            'Expected time till complition (in mins):',
            (proc.time()-irlba.start.time)[3]/iter*(maxit-iter)/60, '\n')
        flush.console()
        j <- 1
        if (iter == 1) 
  ...
  PCAFromSparesMatrix <- function(dtm, n.princomp, max.itter){
    library(irlba)
    ...
    pca <- IRLBA(dtm, nu=200, nv=200,maxit=max.itter)
    data.pca <- pca$u 
